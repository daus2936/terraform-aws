resource "aws_instance" "vprofile-bastion" {
  ...
  key_name               = aws_key_pair.projectterra.key_name
  ...
  provisioner "file" {
    content     = templatefile("templates/db-script.tmpl", { rds-endpoint = aws_db_instance.vprofile-rds.address, dbuser = var.dbuser, dbpass = var.dbpass })
    destination = "/home/ubuntu/vprofile-dbdeploy.sh"
  }
